...
def k_nearest(X, k, obj):
    ...
    # ДЕЛАЕМ ВЫБОРКУ ИЗ МНОЖЕСТВА БЕЗ ПОСДЕДНЕГО СТОЛБЦА
    Xn = X[:, 0:-1]
    # НАХОДИМ СРЕДНИЕ ЗНАЧЕНИЯ ДЛЯ КАЖДОГО СТОЛБЦА
    Xnmean = np.mean(Xn, axis=0)
    # НАХОДИМ СРЕДНЕКВАДРАТИЧНОЕ ОТКЛОНЕНИЕ ДЛЯ КАЖДОГО СТОЛБЦА
    ...
    # СОРТИРУЕМ ИНДЕКСЫ по МЕРЕ УДАЛЕНИЯ ОТ ОБЬЕКТА
    ...
    # array([3, 9])
    # ПОЛУЧАЕМ КЛАССЫ БЛИЖАЙШИХ К СОСЕДЕй
