...
def decision_tree(X, Y, scale, level=0):
    ...
    for i in range(n):
        ...
        if scale[i] == CATEGORICAL:   # категориальный признак
            ...
            info_s = 0
            """
            получаем уникальные элементы из множества со счетчиком вхождений
            """
            unique, counts = np.unique(X[:, i], return_counts=True)
            """
            объединяем массивы в словарь
            """
            uniq_dict = dict(zip(unique, counts))

            """
            проходим по списку уникальных элементов
            """
            for uniq in uniq_dict:
                """
                получаем массив элементов Y для данного значения
                """
                Ycut=Y[X[:,i] == uniq]
                """
                высчитываем info_s
...
def Info(set):
    ...
    if m > 0:
        ...
        for item in uniq_dict:
            ...
            p= uniq_dict[item]/m
            """
            Вычисляем информационную энтропию.
            """
